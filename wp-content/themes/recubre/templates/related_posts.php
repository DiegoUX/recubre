<?php
	global $single_post_config;
?>
<div class="related container<?php if( absint($single_post_config['show_related_phone']) != 1 ) echo " hidden-phone";?>">
	<span class="title"><?php echo stripslashes(esc_attr($single_post_config['related_label'])); ?></span>	
		...
		<?php
			...
			if( !empty( $_cat_list  ))
				$arg=array(
					...
					'posts_per_page' => $single_post_config['num_post_related']
				);
			else
				$arg=array(
				'post_type' => $post->post_type,
...
<script type="text/javascript">
	function switch_flex_slider( windowWidth,orgin_slider ){
		if( windowWidth > 981 && jQuery('.related_post_slider ul > li').length > 4 ) { //4 column
			...
			item_width = jQuery('.related_post_slider').width()/4;
		...
		if( windowWidth <= 980 && windowWidth > 481  && jQuery('.related_post_slider ul > li').length > 3 ) { //3 column
			parrent_div = jQuery('.related_post_slider').parent();
			jQuery('.related_post_slider').hide().remove();
			orgin_slider.clone().appendTo(parrent_div);
		...
		if( windowWidth <= 480 && windowWidth > 321 && jQuery('.related_post_slider ul > li').length > 2 ) { //2 column
			parrent_div = jQuery('.related_post_slider').parent();
			jQuery('.related_post_slider').hide().remove();
			orgin_slider.clone().appendTo(parrent_div);
