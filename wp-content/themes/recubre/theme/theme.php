...
class Theme extends EWAbstractTheme
{
	...
	protected function initArrShortcodes(){
		$this->arrShortcodes = array('bt_accordion','code','column_article','custom_query','embbed_video','image_video',
		'faq','lightbox','listing','quote','sidebar','google_map','slide','style_box','symbol','table','tabs','align','hr','recent_post'
		,'bt_buttons','bt_labels','bt_badges','bt_multitab','bt_tooltips','bt_alerts','bt_progress_bars','bt_carousel','typography','menu','banner','wd_features','wd_testimonial','portfolio');
	}
	//overwrite widget	
	protected function initArrWidgets(){
		$this->arrWidgets = array('flickr','popular','hot_product','recent_post_slider','customrecent','ew_video','emads','custompages','twitterupdate','ew_multitab','Recent_Comments_custom','ew_social','productaz','ew_subscriptions');
	}
	...
	protected function loadImageSize(){
		if ( function_exists( 'add_image_size' ) ) {
		   // Add image size for main slideshow
			...
			add_image_size('blog_thumb',479,479,true); /* image for blog thumbnail */		   
	...
	public function loadOtherJSCSS(){
		...
		if(!is_admin()){			
			wp_register_script( 'techgo', THEME_JS.'/techgo.js',false,false,true);
			wp_enqueue_script('techgo');
		}
	}
}
?>