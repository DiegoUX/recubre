...
if(!class_exists('WP_Widget_Ew_multitab')){
	class WP_Widget_Ew_multitab extends WP_Widget {
		...
		function widget( $args, $instance ) {
			...
			$title_popular = empty( $instance['title_popular'] ) ? __( 'Top Stories','wpdance' ) : $instance['title_popular'];
			...
			$popular = $wpdb->get_results("SELECT view.* FROM {$ppbv_tablename} AS view JOIN {$post_table} AS posts ON view.post_id=posts.ID AND posts.post_type = 'post' ORDER BY views DESC LIMIT 0,$num_popular",OBJECT_K);
	?>
		<div class="container-tabs">
			<div id="tabs-post-sidebar" class="tabs-post-sidebar">
				<ul class="nav nav-tabs wd-widget-multitabs">
					...
					<li><a href="#popular-tab"><span><span><?php echo esc_attr($title_popular); ?></span></span></a></li>   
				...
				<div class="tab-content">
					...
					<!-- Popular Tab -->
					<div id="popular-tab" class="tab-post-content tab-pane">
					<div class='top-left'><div class='top-right'></div></div>
						...
						<ul class="popular-post-list tabs-post-list">
							...
							<li <?php if($i==0) echo "class='first'";if(++$i == count($popular)) echo "class='last'";?>>
								...
								<div class="content">
									<p class="title"><a href="<?php echo esc_url(get_permalink($post->ID)); ?>"><?php echo esc_attr(get_the_title($post->ID)); ?></a></p>
									<div class="entry-meta">
										<span class="view"><i class="icon-eye-open custom-icon"></i><?php echo absint($p->views); ?></span>
										<span class="comment-number"><i class="icon-comments-alt custom-icon"></i><?php comments_number( '0 Comments','1 Comment','% Comments'); ?></span></span>
									</div><!-- .entry-meta -->
								</div>
							</li>
							<?php }?>
			...
			<div class="accordion-tabs wd-widget-multitabs-accordion" id="<?php echo $random_id;?>" style="display:none;">
				<div class="accordion-group">
					<div class="accordion-heading">
						<a class="accordion-toggle" href="#collapseOne-<?php echo $rand_id;?>" data-parent="#<?php echo $random_id;?>" data-toggle="collapse"><span><span><?php echo esc_attr($title_popular); ?></span></span></a>
					</div>
					<div class="accordion-body collapse in" id="collapseOne-<?php echo $rand_id;?>">
							...
							jQuery.debounce(1000, function() {
								if(windowWidth >=200 && windowWidth <= 768){
									jQuery(".widget_multitab > .accordion-tabs").each(function(index,value){
										...
										popularHtml = jQuery(value).siblings('.container-tabs').find('#popular-tab').html();
										...
										jQuery(value).find('#collapseTwo-<?php echo $rand_id;?>').children().html(popularHtml);
	...
	<?php
		...
		function form( $instance ) {
				...
				$title_popular = (strlen(trim($title_popular)) <= 0 ? 'Popular' : $title_popular);
				
				$title_recent = isset($instance['title_popular']) ? esc_attr( $instance['title_recent'] ) : '';
				$title_recent = (strlen(trim($title_recent)) <= 0 ? 'Latest' : $title_recent);
				
				$title_comment = isset($instance['title_popular']) ? esc_attr( $instance['title_comment'] ) : '';
				...
				$num_popular = ($num_popular <= 0 ? 5 : $num_popular);
