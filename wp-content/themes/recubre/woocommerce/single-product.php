							...
							<div id="left-sidebar" class="span6 hidden-phone">
								<div class="left-sidebar-content alpha omega">
									<?php if(!$_right_sidebar) : ?>
										<div class="single_product_summary_end">
											<?php do_action('wd_single_product_summary_end'); ?>
										</div>
										<?php do_action( 'wd_after_single_product_summary' ); ?>
									<?php endif; ?>
								<?php
									if ( is_active_sidebar( $single_prod_datas['left_sidebar'] ) ) : ?>
										<ul class="xoxo">
											<?php dynamic_sidebar( $single_prod_datas['left_sidebar'] ); ?>
										</ul>
								<?php endif; ?>
								</div>
				...
				<?php if( $_right_sidebar  ): ?>
							<div id="right-sidebar" class="span6">
								<div class="right-sidebar-content alpha omega">
									...
									<?php do_action( 'wd_after_single_product_summary' ); ?>
								<?php
									if ( is_active_sidebar( $single_prod_datas['right_sidebar'] ) ) : ?>
										<ul class="xoxo">
											<?php dynamic_sidebar( $single_prod_datas['right_sidebar'] ); ?>
										</ul>
								<?php endif; ?>
								</div>
							</div><!-- end right sidebar -->
