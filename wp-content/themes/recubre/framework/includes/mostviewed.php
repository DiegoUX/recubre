...
function ppbv_display_product($eho = false ){
	global $wpdb,$post;
	$temp = '';
	$temp2 = '';
	$ppbv_tablename = $wpdb->prefix.'popular_by_views';
	$wpdb->flush(); // clense the DB interface
    $data = $wpdb->get_row("SELECT * FROM {$ppbv_tablename} WHERE post_id='{$post->ID}'", ARRAY_A);
	if(!is_null($data)){
		if($data['views'] > 1) 
			$temp = 's';
		if($data['views'] < 10 && $data['views'] > 0)
			$temp2 = '0';
		if(!$eho) {
			echo '<span class="single_views">'.$temp2. $data['views'] . ' view'.$temp.'</span>';
		} else {
			echo '<span class="single_views">'.$temp2. $data['views'].'</span>';
		}
	} else {
		echo '<span class="single_views">0</span>';
	}
}
