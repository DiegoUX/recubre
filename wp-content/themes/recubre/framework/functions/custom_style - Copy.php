<?php
	...
	add_action('wp_head', 'custom_style_inline_script');
	...
	function custom_style_inline_script(){
		...
		if( $enable_custom_font ){
			if( strlen($body_font) > 0 && (int)$body_font != -1 ){?>
				<?php 
					$wd_font_name = $body_font;
					$wd_font_size = $body_font_weight;
					add_action( 'wp_enqueue_scripts', 'wd_load_gg_fonts' );
				?>
			...
			if( strlen($heading_font) > 0 && (int)$heading_font != -1 ){?>
				<?php 
					$wd_font_name = $heading_font;
					$wd_font_size = $heading_font_weight;
					add_action( 'wp_enqueue_scripts', 'wd_load_gg_fonts' );
				?>
			...
			if( strlen($menu_font) > 0 && (int)$menu_font != -1 ){?>
				<?php 
					$wd_font_name = $menu_font;
					$wd_font_size = $menu_font_weight;
					add_action( 'wp_enqueue_scripts', 'wd_load_gg_fonts' );
				?>
			...
			if( strlen($sub_menu_font) > 0 && (int)$sub_menu_font != -1 ){?>
				<?php 
					$wd_font_name = $sub_menu_font;
					$wd_font_size = $sub_menu_font_weight;
					add_action( 'wp_enqueue_scripts', 'wd_load_gg_fonts' );
				?>
