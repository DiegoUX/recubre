	...
	*/
	...
	if(!function_exists('wd_featured_product_slider_function')){
		function wd_featured_product_slider_function($atts,$content){
			...
			if(!(int)$show_rating)
				remove_action( 'woocommerce_after_shop_loop_item', 'woocommerce_template_loop_rating', 6 );				
			if(!(int)$show_label)
				...
				//<![CDATA[
					jQuery(document).ready(function() {
						...
						jQuery(".tabbable").bind('tabs_change',jQuery.debounce( 250, function(){
							...
							jQuery("#<?php echo $_random_id?> > .featured_product_slider_wrapper_inner ul.products").trigger('configuration ',["items.width", 380, true]);
			...
			?>
				...
				<?php woocommerce_product_loop_start(); ?>
					...
					<?php
						...
						if(is_null($_featured_prod) && $_count == 0){ //ko co feature ==> perpage = 5
							$featured_datas['id'] = $product->id;
							get_template_part( 'content', 'product-featured' );
							$per_page = 4;
							if( $_count % 2 == $per_page % 4 ){  //neu count chan
								echo "<li>
							...
							if( $_count < $per_page && function_exists('woocommerce_get_template_part') ){
								woocommerce_get_template_part( 'content', 'product' );
							}	
							if( $_count % 2 == ( ($per_page % 4) + 1 ) ){ //neu count le == 1
								echo "		</ul>
										</div>
									</li>";
							}
			...
			if ( $products->have_posts() ) : ?>
				...
				//<![CDATA[
					jQuery(document).ready(function() {
						...
						/*
						var jQuery('elems') = jQuery('#<?php echo $_random_id?> > .featured_product_slider_wrapper_inner > ul
						, #<?php echo $_random_id;?>_prev
						, #<?php echo $_random_id;?>_next');
						var jQuery('image') = jQuery('#<?php echo $_random_id?> > .featured_product_slider_wrapper_inner > ul li:first')
					 
						jQuery(window).bind( 'resize.example', function() {
							var height = jQuery('image').outerHeight( true );
