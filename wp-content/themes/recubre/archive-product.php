		...
		<div id="content" class="container" role="main">
			<div id="main">	
				...
				<?php
					...
					if( isset($category_prod_datas) ){
						$_layout_config = explode("-",$category_prod_datas['cat_layout']);
					}else{
						$category_prod_datas['cat_right_sidebar'] = 'category-widget-area';
						$category_prod_datas['cat_left_sidebar'] = 'category-widget-area';
				...
				<?php if( $_left_sidebar ): ?>
							<div id="left-sidebar" class="span6 hidden-phone">
								...
								<?php
									if ( is_active_sidebar( $category_prod_datas['cat_left_sidebar'] ) ) : ?>
										<ul class="xoxo">
											<?php dynamic_sidebar( $category_prod_datas['cat_left_sidebar'] ); ?>
										</ul>
								<?php endif; ?>
								</div>
				...
				<div id="main_content" class="<?php echo $_main_class?>">
					...
					<div class="alpha omega">				
						...
						?>
							...
							<?php
								if( isset($category_prod_datas['cat_custom_content']) && strlen($category_prod_datas['cat_custom_content']) > 0 ){
									echo "<div class='cat_custom_content'>";
									echo do_shortcode (stripslashes(htmlspecialchars_decode( base64_decode($category_prod_datas['cat_custom_content']) )) );
									echo "</div>";
								}
							?>
							...
							<?php 
								...
								if( absint($category_prod_datas['cat_columns']) > 0 ){
									$woocommerce_loop['columns'] = absint($category_prod_datas['cat_columns']);
								}
							?>
				...
				<?php if( $_right_sidebar  ): ?>
							<div id="right-sidebar" class="span6">
								...
								<?php
									if ( is_active_sidebar( $category_prod_datas['cat_right_sidebar'] ) ) : ?>
										<ul class="xoxo">
											<?php dynamic_sidebar( $category_prod_datas['cat_right_sidebar'] ); ?>
										</ul>
								<?php endif; ?>
								</div>
